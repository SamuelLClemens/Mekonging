...
async function renderVault(body) {
  ...
  if (docs.length && !store.profile.prefs.vaultBackupDone) {
    body.append(h('div', { class: 'card', style: 'border:1px solid var(--orange)' }, [
      ...
      h('p', { class: 'muted', style: 'margin: var(--sp-1) 0 var(--sp-2)' }, 'Protects these from an update, reset, or lost phone.'),
  ...
  docs.forEach((d) => {
    ...
    const reveal = h('div', { style: 'flex-basis:100%;margin-top: var(--sp-1h);display:none' });
      ...
      ? h('button', { class: 'chip', onclick: async () => {
          ...
          try {
            ...
            const box = h('div', { class: 'card', style: 'margin: 0' }, [
              ...
              h('button', { class: 'chip', style: 'margin-top: var(--sp-1h)', onclick: () => { try { navigator.clipboard.writeText(text); } catch { /* no clipboard */ } } }, 'Copy'),
            ]);
...
function recoveryCodeCard(body, code, opts) {
  ...
  return h('div', { class: 'card', style: 'border:2px solid var(--orange)' }, [
    ...
    h('div', { style: 'margin: var(--sp-3) 0;padding: var(--sp-4);text-align:center;font-size:1.15rem;letter-spacing:1px;font-family:monospace;user-select:all;word-break:break-all;background:var(--card-2, rgba(0,0,0,.06));border-radius:10px' }, code),
    h('div', { class: 'row-between' }, [
    ...
    h('p', { class: 'disclaimer', style: 'margin-top: var(--sp-3)' }, 'Keep it private and separate from your device — a password manager, a note at home, or written down. Anyone with this code can open your vault.'),
...
function vaultSetupCard(body) {
  ...
  return h('div', { class: 'card' }, [
    ...
    h('p', { class: 'muted', style: 'margin: var(--sp-3) 0 var(--sp-1)' }, 'Moving from another device? Restore your encrypted backup, then unlock it with the same passcode.'),
    vaultRestoreControl(body),
  ]);
}
function vaultUnlockCard(body, hintText) {
  ...
  return h('div', { class: 'card' }, [
    ...
    hintText ? h('p', { class: 'muted', style: 'margin: var(--sp-1) 0 0' }, `💡 Reminder: ${hintText}`) : null,
...
function forgottenPasscodeDetails(body) {
  ...
  return h('details', { class: 'filters-collapse', style: 'margin-top: var(--sp-3)' }, [
    ...
    h('div', {}, [
      ...
      h('p', { class: 'muted', style: 'margin-top: var(--sp-4)' }, 'No recovery code? If you saved an encrypted backup, restore it and unlock with that backup’s passcode.'),
      vaultRestoreControl(body),
